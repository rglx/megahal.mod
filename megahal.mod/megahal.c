 ...
 *
 * Additions and changes by rglx:
 *
 * Ver 3.7.1 Sept 2021
 * - changed pub bindings to pubm
 * - revised companion script heavily
 * - didn't add version increment because nothing really changed brain-wise
 *
 *
...
#define BOTNICK "bots"
#define BOTNICK2 "bots:"
#define BOTNICK3 "bots,"
...
{
  {BOTNICK, "", pub_megahal, "mega"},
  {BOTNICK2, "", pub_megahal, "mega"},
  {BOTNICK3, "", pub_megahal, "mega"},
  {"*", "", pub_megahal2, "mega"},
  {NULL, NULL, NULL, NULL}
};
...
char *megahal_start(Function * global_funcs)
{
	...
	putlog(LOG_MISC, "*", "MegaHAL v%s loaded.", VER);
...
static int dcc_megaver(struct userrec *u, int idx, char *text)
{
	...
	if(!floodcheck())
		return 0;
	dprintf(idx, "MegaHAL module v%s\n", VER);
	return 0;
}
...
static int pub_megaver(char *nick, char *host, char *hand, char *channel, char *text)
{
	...
	if(!floodcheck())
		return 0;
	dprintf(DP_HELP, "PRIVMSG %s :MegaHAL module v%s\n", channel, VER);
	return 0;
}
